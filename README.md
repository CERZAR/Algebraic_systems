...
Циклическая группа - это группа, которая может быть порождена одним элементом. Другими словами, каждый элемент группы может быть выражен как несколько степеней этого порождающего элемента. Если обозначить порождающий элемент за $a$, то любой элемент группы $G$ может быть представлен как $a^n$, где $n$ - целое число.

Основные положения теоремы:

1. Конечные циклические группы: Если циклическая группа имеет конечный порядок $n$, то она изоморфна группе остатков по модулю $n$, обозначаемой как $\mathbb{Z}/n\mathbb{Z}$ или $\mathbb{Z}_n$. Элементы этой группы состоят из классов эквивалентности чисел по модулю $n$ с операцией сложения. Это означает, что структура конечной циклической группы полностью определяется её порядком $n$, и она содержит элементы порядка $d$ для каждого делителя $d$ числа $n$.

2. Бесконечные циклические группы: Любая бесконечная циклическая группа изоморфна аддитивной группе целых чисел $\mathbb{Z}$, с операцией сложения. В такой группе для каждого элемента $a$ существует обратный элемент $-a$, и множество всех элементов образуется через $a^n$, где $n$ может принимать любое целочисленное значение.

**Следствие из теоремы**: Группа $G$ является циклической тогда и только тогда, когда существует элемент $g \in G$ такой, что любой другой элемент $h$ группы $G$ можно представить как $h=g^n$ для некоторого целого числа $n$.

Таким образом, теорема о строении циклических групп позволяет классифицировать все циклические группы на два больших класса: конечные, каждая из которых изоморфна некоторому кольцу вычетов, и бесконечные, которые изоморфны группе целых чисел.

### Конечные циклические группы
Теорема: Каждая конечная циклическая группа порядка $n$ изоморфна группе $\mathbb{Z}/n\mathbb{Z}$.

Доказательство:

Пусть $G = \langle a \rangle$ - циклическая группа порядка $n$, что означает, что $a^n = e$, где $e$ – нейтральный элемент группы, и для любого $0 < m < n$, $a^m \neq e$.

Рассмотрим гомоморфизм $\phi$ из $\mathbb{Z}$ в $G$, заданный как $\phi(k) = a^k$. Так как $a^n = e$, можно заметить, что $\phi(k + n) = a^{k + n} = a^k \cdot a^n = a^k = \phi(k)$, что показывает, что $\phi$ индуцированно отношением эквивалентности на $\mathbb{Z}$, порожденным наличием деления на $n$. То есть, $\phi$ индуцирует гомоморфизм из $\mathbb{Z}/n\mathbb{Z}$ в $G$.

Поскольку $a^k = e$ тогда и только тогда, когда $n$ делит $k$, ядро этого гомоморфизма состоит из класса $n\mathbb{Z}$. По теореме о гомоморфизме, $\mathbb{Z}/n\mathbb{Z}$ изоморфно подгруппе $G$, которая совпадает с $G$, так как $a^k$ порождает все $G$.

### Пример с конечной циклической группой
Рассмотрим циклическую группу $\mathbb{Z}/12\mathbb{Z}$, которая представляет собой группу остатков по модулю 12. Элементы этой группы - это числа от 0 до 11, и операция, определенная на группе, это сложение по модулю 12.

Эта группа является примером конечной циклической группы порядка 12, где каждый элемент может быть получен как степень одного из её элементов, который мы назовём генератором. Например, 1 является генератором этой группы, потому что каждый элемент группы может быть выражен как степень 1 по модулю 12. То есть $1, 2, 3, \dots, 11$ можно получить, сложив 1 с самим собой нужное количество раз по модулю 12.

### Бесконечные циклические группы
Теорема: Каждая бесконечная циклическая группа изоморфна $(\mathbb{Z}, +)$.

Доказательство:

Пусть $G = \langle a \rangle$ – бесконечная циклическая группа. Рассмотрим отображение $\phi: \mathbb{Z} \rightarrow G$ такое, что $\phi(k) = a^k$. Это отображение является гомоморфизмом, так как $\phi(k + m) = a^{k + m} = a^k \cdot a^m = \phi(k) \cdot \phi(m)$.

Теперь нужно показать, что отображение $\phi$ является биекцией. Инъективность следует из того факта, что если $a^k = a^m$, то $a^{k-m} = e$, что возможно только если $k = m$ поскольку группа бесконечна и нет наименьшего положительного $n$ такого, что $a^n = e$. Сюръективность следует из того, что любой элемент $G$ является некоторой степенью $a$.

Таким образом, отображение $\phi$ является изоморфизмом между $\mathbb{Z}$ и $G$, что доказывает утверждение.

Эти доказательства показывают, что структура циклических групп полностью определяется их порядком, и все они либо конечны и изоморфны $\mathbb{Z}/n\mathbb{Z}$ для некоторого $n$, либо бесконечны и изоморфны $\mathbb{Z}$.

### Пример с бесконечной циклической группой
В качестве примера бесконечной циклической группы возьмём группу целых чисел $\mathbb{Z}$ с операцией сложения. Эта группа является циклической, потому что любой элемент группы может быть выражен через сложение или вычитание одного элемента (например, 1 или -1) определённое количество раз.

1 является генератором группы, так как сложив 1 достаточное количество раз, можно получить любое положительное целое число. Аналогично, сложив -1 достаточное количество раз, можно получить любое отрицательное целое число. Таким образом, целые числа $\mathbb{Z}$ образуют бесконечную циклическую группу, генерируемую 1 или -1.
